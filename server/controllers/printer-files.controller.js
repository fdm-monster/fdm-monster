...
import printerFilesController from "./validation/printer-files-controller.validation";
...
const { validateInput, getScopedPrinter, validateMiddleware } = validators;
const { getFilesRules, getFileRules, uploadFileRules, fileUploadCommandsRules, selectAndPrintFileRules, localFileUploadRules, moveFileOrFolderRules, createFolderRules } = printerFilesController;
const { ValidationException, NotFoundException } = runtime;
const { printerLoginToken, printerResolveMiddleware } = printer;
const { ROLES, PERMS } = authorization;
class PrinterFilesController {
    ...
    async #getUploadedFile(req, res, storeAsTempFile = true) {
        ...
        await new Promise((resolve, reject) => multerMiddleware(req, res, (err) => {
            if (err) {
                return reject(err);
            }
            resolve();
    ...
    async uploadFile(req, res) {
        ...
        const response = await this.#octoPrintApiService.uploadFileAsMultiPart(printerLogin, files[0], commands, token);
