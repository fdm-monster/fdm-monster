...
import PrinterGroupModel from "../models/PrinterGroup";
...
} from "./validators/printer-group-service.validation";
    ...
    async syncPrinterGroups() {
        ...
        for (const [groupName, printers] of Object.entries(printersGrouped)) {
            ...
            const printerIds = printers.map((p) => {
                const printerPort = parseInt(new URL(p.printerURL).port);
                const index = (printerPort || 80) - 80;
                return {
                    printerId: p._id,
                    location: index
                };
