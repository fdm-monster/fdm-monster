...
class TaskManagerService {
    ...
    validateInput(taskId, workload, schedulerOptions) {
        ...
        if (typeof workload !== "function") {
            if (typeof workload !== "string") {
                throw new JobValidationException(`${prefix} is not a callable nor a string dependency to lookup. It can't be scheduled.`, taskId);
            }
            let resolvedService;
            try {
                ...
                if (e instanceof AwilixResolutionError) {
                    throw new JobValidationException(`${prefix} had an awilix dependency resolution error. It can't be scheduled without fixing this problem. Inner error:\n` +
                        e.stack, taskId);
                } else {
                    throw new JobValidationException(`${prefix} is not a registered awilix dependency. It can't be scheduled. Error:\n` +
                        e.stack, taskId);
                }
            }
        ...
        if (schedulerOptions?.runDelayed &&
    ...
    scheduleDisabledJob(taskId, failIfEnabled = true) {
        ...
        if (schedulerOptions?.disabled !== true) {
            if (failIfEnabled) {
                throw new JobValidationException(`The requested task with ID ${taskId} was not explicitly disabled and must be running already.`);
            }
            return;
        }
    ...
    #scheduleEnabledPeriodicJob(taskId) {
        ...
        if (!taskState?.timedTask || !taskState?.options) {
            throw new JobValidationException(`The requested task with ID ${taskId} was not registered properly ('timedTask' or 'options' missing).`);
        }
        const schedulerOptions = taskState?.options;
        ...
        if (!schedulerOptions.disabled) {
            this.#logger.info(`Task '${taskId}' was scheduled (runImmediately: ${!!schedulerOptions.runImmediately}).`);
