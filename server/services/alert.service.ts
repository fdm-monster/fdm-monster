...
class AlertService {
    ...
    async convertMessage(printer, message, historyID) {
        ...
        let progress: any = {};
        if (!printer?.progress?.completion) {
        ...
        if (message.includes("[ETA]")) {
            ...
            if (typeof printer.progress !== "undefined" && printer.progress.printTimeLeft !== null) {
                ...
                let futureDateString = new Date(dateNumber + printer.progress.printTimeLeft * 1000).toDateString();
                let futureTimeString = new Date(dateNumber + printer.progress.printTimeLeft * 1000).toTimeString();
        ...
        if (message.includes("[Tool0Temp]")) {
            if (typeof printer.temps != "undefined" &&
                ...
                message = message.replace(/\[Tool0Temp\]/g, "T:" +
