...
class ServerUpdateService {
    ...
    static filterGithubReleases(releases, prerelease = false, tag_name = null) {
        return releases.find((r) => r.draft === false &&
    ...
    async syncLatestRelease(includePre = false) {
        ...
        if (githubReleases?.length > 0) {
            ...
            this.#installedReleaseFound = !!ServerUpdateService.filterGithubReleases(githubReleases, includePre, this.#serverVersion);
    ...
    getUpdateNotificationIfAny() {
        if (this.#notificationReady === true && this.#airGapped !== true) {
            ...
            return {
                ...
                message: "You can update 3DH Server to the latest version available: " +
    ...
    checkReleaseAndLogUpdate() {
        if (!!this.#lastReleaseCheckFailed) {
            this.#logger.error("Cant check release as it was not fetched yet or the last fetch failed. Call and await 'syncLatestRelease' first.");
            return;
        }
        ...
        if (!this.#installedReleaseFound) {
            this.#logger.info(`\x1b[36mAre you a god? A new release ey? Bloody terrific mate!\x1b[0m
    ...
    Appreciate the hard work, you rock!`);
        ...
        if (!!packageVersion && packageVersion !== latestReleaseTag) {
            if (!!this.#airGapped) {
                this.#logger.warning(`Installed release: ${packageVersion}. Skipping update check (air-gapped/disconnected from internet)`);
            } else {
                this.#logger.info(`Update available! New version: ${latestReleaseTag} (prerelease: ${latestRelease.prerelease})`);
            }
        } else if (!packageVersion) {
            return this.#logger.error("Cant check release as package.json version environment variable is not set. Make sure 3DH Server is run from a 'package.json' or NPM context.");
        } else {
...
export default ServerUpdateService;
