class InfluxDbHistoryService {
    MEASUREMENT_NAME = "HistoryInformation";
    ...
    //     let currentArchive = _.findIndex(historyArchive, function (o: any) {
    ...
    //         const trueStartDate = Date.parse(`${startDateSplit[2]} ${startDateSplit[1]} ${startDateSplit[3]} ${startDateSplit[5]}`);
    //         const trueEndDate = Date.parse(`${endDateSplit[2]} ${endDateSplit[1]} ${endDateSplit[3]} ${endDateSplit[5]}`);
    //         let currentState = " ";
    ...
    //             printerData["job_resends"] = `${workingHistory.resend.count} / ${workingHistory.resend.transmitted / 1000}K (${workingHistory.resend.ratio.toFixed(0)})`;
    //         }
...
export default InfluxDbHistoryService;
