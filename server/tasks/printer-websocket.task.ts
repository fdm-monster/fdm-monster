...
class PrinterWebsocketTask {
    ...
    async run() {
        ...
        if (newPrinterStates.length !== printerStates.length) {
            this.#logger.info(`Attempted websocket connections taking ${duration}ms. ${newPrinterStates.length} adapters need retry (before: ${printerStates.length}).`);
        }
    ...
    #resetPrinterErrorCount(key, printerId) {
        delete this.#errorCounts[key][printerId];
    }
