...
describe("PrinterStore", () => {
  ...
  it("should return with empty octoprint versions array", async () => {
    const returnedStats = await printerStore.getOctoPrintVersions();
    expect(returnedStats).toEqual([]);
  });

  it("should avoid adding invalid printer", async () => {
    await expect(async () => await printerStore.addPrinter({})).rejects.toBeInstanceOf(
      ValidationException
    );

    expect(() => printerStore.addPrinter(invalidNewPrinterState)).rejects.toHaveErrors({
      apiKey: {
        rule: "length",
      },
      webSocketURL: {
    ...
    await expect(async () => await printerStore.addPrinter(weakNewPrinter)).rejects.toBeInstanceOf(
      ValidationException
    );

    await expect(async () => await printerStore.addPrinter(weakNewPrinter2)).rejects.toBeDefined();
  });

  it("should be able to add and flatten new printer", async () => {
    let frozenObject = await printerStore.addPrinter(validNewPrinterState);
    const flatState = printerStore.getPrinterFlat(frozenObject.id);
    expect(Object.isFrozen(flatState)).toBeTruthy();
  });

  it("should be able to add printer - receiving an state object back", async () => {
    let unfrozenStateInstance = await printerStore.addPrinter(validNewPrinterState);
  ...
  it("should load printerStore with a saved printer", async () => {
    await printerStore.addPrinter(validNewPrinterState);
    await printerStore.loadPrinterStore();
  });

  it("should update api user from printerStore", async () => {
    const newApiUserName = "testname";
    const newPrinter = await printerStore.addPrinter(validNewPrinterState);
    const result = await printerStore.updateApiUserName(newPrinter.id, newApiUserName);
  ...
  it("should skip teardown test printer when already undefined", async () => {
    await printerStore.deleteTestPrinter();
  });

  it("should get undefined test printer from store", async () => {
    expect(printerStore.getTestPrinter()).toBeUndefined();
  });
