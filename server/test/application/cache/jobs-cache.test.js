...
describe("getCompletionDate", () => {
    it("should calculate formatted completion date", async function () {
        ...
        const refCutoffDate = legacyUUTGetCompletionDate(10, 99).substring(0, completionDate.length - 1 - 4);
...
describe("JobsCache", () => {
    ...
    it("should not tolerate saving without id", () => {
        expect(() => jobsCache.getPrinterJob()).toThrow("Job Cache cant get a null/undefined printer id");
    });
    it("should not tolerate updating with wrong id", () => {
        expect(() => jobsCache.updatePrinterJob()).toThrow(`this printer ID undefined is not known. Cant update printer job cache.`);
    });
