...
} from "../../constants/cleaner.constants.js";
...
class DashboardStatisticsCache {
    ...
    async generatePrinterStatistics(id) {
        ...
        const i = _.findIndex(currentPrinters, function (o: any) {
    ...
    async statisticsStart() {
        ...
        for (let p = 0; p < this.#printersInformation.length; p++) {
            ...
            if (typeof printer !== "undefined") {
                ...
                if (typeof printer.printerState !== "undefined") {
                    ...
                    if (typeof tools !== "undefined" && tools !== null) {
                        ...
                        for (let k = 0; k < keys.length; k++) {
                            if (typeof printer.currentProfile !== "undefined" &&
                        ...
                        actualString += `" class="${checkTempRange(printer.printerState.colour.category, totalToolTarget, totalToolActual, printer.otherSettings.temperatureTriggers.heatingVariation, printer.otherSettings.temperatureTriggers.coolDownf)} heatMapLeft"></div>`;
                        actualString += '<div title="';
                        for (let r = 0; r < rightString.length; r++) {
                            actualString += `${rightString[r]}`;
                        }
                        actualString += `" class="${checkTempRange(printer.printerState.colour.category, totalOtherTarget, totalOtherActual, printer.otherSettings.temperatureTriggers.heatingVariation, printer.otherSettings.temperatureTriggers.coolDown)} heatMapLeft"></div></div>`;
                    ...
                    heatProgress.push(`<div title="${printer.printerName}: ${progress}%" class="bg-${mapProgressToColor(progress)} heatMap"></div>`);
                }
                ...
                heatUtilisation.push(`<div title="${printer.printerName}: ${percentUp.toFixed(0)}" class="bg-${mapProgressToColor(percentUp)} heatMap"></div>`);
            }
        }
        ...
        const totalGlobalTemp = totalGlobalToolTempActual + totalGlobalBedTempActual + totalGlobalChamberTempActual;
