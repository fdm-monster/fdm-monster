...
import {NotFoundException, NotImplementedException, ValidationException} from "../exceptions/runtime.exceptions.js";
import {validateInput} from "../handlers/validators.js";
import {createTestPrinterRules} from "./validation/create-test-printer.validation.js";
...
class PrintersStore {
    ...
    constructor({settingsStore, printerStateFactory, eventEmitter2, printerService, loggerFactory}) {
    ...
    _validateState() {
        if (!this.#printerStates) {
            throw new Error("PrintersStore was not loaded. Cant fire printer loading action. Call 'loadPrintersStore' first.");
        }
    }
    ...
    async updateApiUserName(printerId, username) {
        ...
        if (!username?.length)
            throw new ValidationException([{error: "Username must be non-empty"}]);
    ...
    reconnectOctoPrint(id, lazy = true) {
        if (!lazy) {
            ...
            throw new NotImplementedException("Eager (lazy==true) reconnect OctoPrint mode is not implemented yet.");
        }
