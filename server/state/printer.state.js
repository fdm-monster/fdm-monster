...
class PrinterState {
    ...
    bindWebSocketAdapter(adapterType) {
        ...
        if (!this.#sessionUser || !this.#sessionKey) {
            throw new Error("Printer State 'bindWebSocketAdapter' was called without 'sessionUser' or 'sessionKey' set-up correctly.");
        }
    ...
    connectAdapter() {
        if (!this.#websocketAdapter) {
            throw new Error(`The websocket adapter was not provided. Please reset it first with 'bindWebSocketAdapter' to connect to it.`);
        }
        this.#messageSubscription = this.#messageStream.subscribe((r) => {
    ...
    setApiAccessibility(accessible, retryable, reason) {
        if (!accessible) {
            if (!retryable) {
                this.#logger.warning(`Printer API '${this.getName()}' was marked as inaccessible. Reason: '${reason}'. Please check connection settings.`);
            }
        }
