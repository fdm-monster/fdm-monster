...
describe(YamlService.name, () => {
  it("should import yaml from version 1.3.1", async () => {
    ...
    const floors = await floorService.list();
    expect(floors.find((f) => f.name === "Default Floor1_3_1")).toBeDefined();
  });
  ...
  it("should import yaml from version 1.5.0", async () => {
    ...
    const floors = await floorService.list();
    expect(floors.find((f) => f.name === "Default Floor1_5_0")).toBeDefined();
  });
  ...
  it("should import 1.5.2 yaml", async () => {
    ...
    expect(floor.printers.find((p) => p.printerId === printer)).toBeDefined();
  });
});
