...
export class FileAnalysisService {
  ...
  async analyzeFile(filePath: string): Promise<{
    ...
    this.logger.log(`Analyzing file: ${path.basename(filePath)} (format: ${fileFormat}, ext: ${ext})`);
   ...
   */
  async estimateAnalysisTime(filePath: string): Promise<number> {
    ...
    if (ext === ".bgcode") return sizeMB * 0.3;
    if (ext === ".3mf") return sizeMB * 0.5;
    return sizeMB * 0.2;
  }
}
