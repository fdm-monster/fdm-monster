...
export class PrintJobService implements IPrintJobService {
  ...
  async handlePrintStarted(
    ...
    // If the same file is already printing, just return it (don't create duplicate)
    if (existingJob && existingJob.fileName === fileName && !jobId) {
      this.logger.debug(`Job ${existingJob.id} already printing file ${fileName}, returning existing job`);
      return existingJob;
    }
