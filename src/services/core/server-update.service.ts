...
import { InternalServerException, NotImplementedException, ValidationException } from "@/exceptions/runtime.exceptions";
...
export class ServerUpdateService {
  ...
  constructor({ simpleGitService, loggerFactory }: { simpleGitService: SimpleGit; loggerFactory: ILoggerFactory }) {
    this.simpleGitService = simpleGitService;
    this.logger = loggerFactory(ServerUpdateService.name);
  }

  async restartServer() {
    throw new NotImplementedException("Restart is not implemented yet");
  }
