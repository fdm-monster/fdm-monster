...
function printPreBootMessage(error = null) {
  ...
  if (!error) {
    ...
    logger.error(error);
    logger.error(
      `Booted in SAFE MODE with a startup error at ${startMoment}! Please visit http://${APP_HOST}:${APP_PORT} to see more.`
    );
  }
}
...
async function testDatabase() {
  ...
  let url = `${db.type}://${db.username}:${db.password}@${db.hostname}/${db.database}?authSource=admin`;
  if (!db.username || !db.password) {
    url = `${db.type}://${db.hostname}/${db.database}`;
  }
...
function logRoutes(app: INestApplication) {
  ...
  logger.warn(
    "Want to suppress API/MVC routes? Remove or set the env variable 'PRINT_ROUTES_DEBUG' to anything but 'true'"
  );
}
