...
@Injectable()
export class SafeEventSubscribersLoader implements OnApplicationBootstrap, OnApplicationShutdown {
  private readonly injector = new Injector();
  private logger = new Logger("SafeEventSubscribersLoader");
    ...
    [...providers, ...controllers]
      ...
      .forEach((wrapper: InstanceWrapper) => {
        ...
        this.metadataScanner.scanFromPrototype(instance, prototype, (methodKey: string) =>
          this.subscribeToEventIfListener(instance, methodKey, isRequestScoped, wrapper.host as Module)
        );
      });
  }
        ...
        listenerMethod(
          ...
          async (...args: unknown[]) => {
            try {
              ...
              this.logger.error(`Error caught in event subscriber - ${errorSummary(e)}`);
  ...
  private registerRequestScopedListener(eventListenerContext: {
    ...
    listenerMethod(
      ...
      async (...args: unknown[]) => {
        ...
        try {
          return await contextInstance[listenerMethodKey].call(contextInstance, ...args);
        } catch (e) {
          this.logger.error(`Error caught in request scoped event subscriber - ${errorSummary(e)}`);
          this.sentry.instance().captureException(e);
        }
      },
      options
  ...
  private registerEventPayloadByContextId(eventPayload: unknown[], contextId: ContextId) {
    /*
